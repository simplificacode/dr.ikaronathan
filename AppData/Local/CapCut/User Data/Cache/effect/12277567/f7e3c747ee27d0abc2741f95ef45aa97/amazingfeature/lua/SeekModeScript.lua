...
function SeekModeScript:onEvent(sys, event)
    if event.type == Amaz.AppEventType.SetEffectIntensity then
        if event.args:get(0) == "intensity" then
            globalpercentage.small_head_value = event.args:get(1)
            self.bodyIntensity = globalpercentage.small_head_value
        end
    end
end
...
function SeekModeScript:seekToTime(comp, time)
    ...
    globalpercentage.small_head_value = self.bodyIntensity
    updateAllScale()
...
function SeekModeScript:updateFaceInfo()
    ...
    if faceCount > 0 then
        ...
        for j = 1, 5 do
            ...
            faceKeyPoints[j].y = img_height - faceKeyPoints[j].y*img_height
        end
