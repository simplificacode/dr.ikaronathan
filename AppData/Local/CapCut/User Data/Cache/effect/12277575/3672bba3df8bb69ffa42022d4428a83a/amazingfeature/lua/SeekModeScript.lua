...
function SeekModeScript:onEvent(sys, event)
    if event.type == Amaz.AppEventType.SetEffectIntensity then
        if event.args:get(0) == "intensity" then
            globalpercentage.slim_waist_value = event.args:get(1)
            self.bodyIntensity = globalpercentage.slim_waist_value
        end
    end
end
...
function SeekModeScript:seekToTime(comp, time)
    ...
    globalpercentage.slim_waist_value = self.bodyIntensity
    updateAllScale()
