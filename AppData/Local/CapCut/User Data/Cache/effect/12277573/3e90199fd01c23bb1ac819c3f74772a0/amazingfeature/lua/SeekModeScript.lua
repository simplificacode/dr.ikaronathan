--@input float curTime = 0.50 {"widget":"slider","min":0.0,"max":1.0}
...
function SeekModeScript.new(construct, ...)
    ...
    self.height = 0
    -- self.material = nil
    globalpercentage.stretch_leg_value = 0.0
...
function SeekModeScript:onUpdate(comp, detalTime)
    ...
    --     globalpercentage.stretch_leg_value = props:get("curTime")
...
function SeekModeScript:onEvent(sys, event)
    if event.type == Amaz.AppEventType.SetEffectIntensity then
        if event.args:get(0) == "intensity" then
            -- cccccccc
            globalpercentage.stretch_leg_value = event.args:get(1)
            -- print("skeleton intensity:", globalpercentage.stretch_leg_value)
            self.bodyIntensity = globalpercentage.stretch_leg_value
        end
    end
end
...
function SeekModeScript:seekToTime(comp, time)
    ...
    globalpercentage.stretch_leg_value = self.bodyIntensity
    updateAllScale()
