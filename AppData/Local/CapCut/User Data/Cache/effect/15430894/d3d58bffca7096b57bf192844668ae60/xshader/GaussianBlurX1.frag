...
void main()
{
    float range = v_bloomPara.y*fit_scale;

    float theta = 0.0 * 3.1415926 / 180.;
    vec2 ratio = vec2(720.0) * u_ScreenParams.xy / min(u_ScreenParams.x, u_ScreenParams.y);
    vec2 dir = vec2(cos(theta), sin(theta)) / ratio;
    //vec4 color = gaussianBlur(u_TurbulentInputTex, uv0, dir, u_Strength);
    vec4 color = gaussianBlur(u_TurbulentInputTex, uv0, dir, range);
    gl_FragColor = color;
}
